...
export const updateDepartment = async (data, id, token) => {
  try {
    api.defaults.headers.common["Authorization"] = `Bearer ${token}`;
...
// Customer Department APIs
export const getCustomerDepartments = async (customerId, token) => {
  try {
    api.defaults.headers.common["Authorization"] = `Bearer ${token}`;
    const response = await api.get(`/customers/${customerId}/departments`);
    return response.data;
  } catch (error) {
    console.log(error);
    return {
...
export const addCustomerDepartment = async (customerId, data, token) => {
  try {
    api.defaults.headers.common["Authorization"] = `Bearer ${token}`;
